...
O desenvolvimento do projeto seguirá um fluxo estruturado, desde o tratamento dos dados históricos até a comparação entre o cenário atual e o cenário otimizado.

```mermaid
flowchart TD
    A["📂 Dados históricos do IPEM"] --> B["🧹 Limpeza e tratamento dos dados"]
    B --> C["📊 Análise do cenário histórico"]
    C --> D["🗺️ Matriz de distâncias e tempos"]
    D --> E["🧮 Modelagem em Pesquisa Operacional"]
    E --> F["🐍 Otimização em Python"]
    F --> G["⚙️ Construção do cenário otimizado"]
    G --> H["🔍 Comparação: Histórico x Otimizado"]
